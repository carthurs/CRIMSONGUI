...
Program:   VMTK
Module:    $RCSfile: crimsonTetGenWrapper.cxx,v $
Language:  C++
...
vtkStandardNewMacro(crimsonTetGenWrapper);

crimsonTetGenWrapper::crimsonTetGenWrapper()
{
  ...
  this->MaxRatio = 2.0;         //    number after -q, 2.0
  ...
  this->OptimizationLevel = 0;  // -O switch, 0
...
int crimsonTetGenWrapper::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  if (this->Quality)
    ...
    sprintf(buffer,"%f",this->MaxRatio);
  ...
  if (this->OptimizationLevel != 0)
    {
    tetgenOptionString += "O";
    sprintf(buffer,"%i",this->OptimizationLevel);
    tetgenOptionString += buffer;
    tetgenOptionString += "/7";
    tetgenOptionString += "o/";
...
// tetgenOptionString += "Y";  
  ...
  for (i=0; i<numberOfPoints; i++)
    ...
    if (sizingFunctionArray)
      ...
      if (in_tetgenio.pointmtrlist[i] == 0.0)
        {
        in_tetgenio.pointmtrlist[i] = 1e-6;
        }
      }
    }
  ...
  if (numberOfFacets > 0) {
      if (!this->Refine) {
          ...
          in_tetgenio.numberoftrifaces = numberOfFacets;
          in_tetgenio.trifacelist = new int[numberOfFacets * 3];
          in_tetgenio.trifacemarkerlist = new int[numberOfFacets];
          ...
          for (i = 0; i < numberOfFacets; i++) {
              ...
              in_tetgenio.trifacelist[i * 3 + 2] = pts[2];

              if (facetMarkerArray) {
                  in_tetgenio.trifacemarkerlist[i] = static_cast<int>(facetMarkerArray->GetComponent(facetCellIds->GetId(i), 0));
              } else {
                  in_tetgenio.trifacemarkerlist[i] = 0;
              }
          }
      }
  }
  ...
  strcpy(tetgenOptions, this->FullString.empty() ? tetgenOptionString.c_str() : this->FullString.c_str());
...
void crimsonTetGenWrapper::PrintSelf(ostream& os, vtkIndent indent)
{
